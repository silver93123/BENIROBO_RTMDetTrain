...
인자:
    --dataset   데이터셋 폴더명만 입력 (필수)
                공통 경로: <프로젝트 루트>/data/dataset/ (스크립트 위치 기준 자동 계산)
                예) 20260521_114500
                    → <프로젝트 루트>/data/dataset/20260521_114500 로 자동 완성
...
# 데이터셋 공통 루트 경로
# 예전: 다른 프로젝트의 절대경로가 하드코딩되어 있었음 (다른 PC/폴더로 옮기면 깨짐)
# 지금: 이 스크립트 자신의 위치(scripts/의 부모 = 프로젝트 루트) 기준 상대경로로 계산
#       -> 프로젝트 폴더를 통째로 옮겨도 항상 <프로젝트 루트>/data/dataset 를 가리킴
DATASET_BASE = ROOT / "data" / "dataset"
